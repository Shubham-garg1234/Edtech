...
export default function usePaymentService() {
  ...
  const handlePayment = async (amount) => {
    try {
      const response = await fetch("http://localhost:8081/api/razorpay/v1/createOrder?amount="+amount+"&currency=INR", {
        method: "GET",
        headers: {
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            const verifyResponse = await fetch("http://localhost:8081/api/razorpay/v1/verify", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
